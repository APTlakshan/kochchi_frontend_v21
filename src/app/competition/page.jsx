...
function CompetitionPage() {
  ...
  useEffect(() => {
    const fetchCompetitions = async () => {
      try {
        const response = await fetch('https://kochchibazaar.lk/api/competition/', {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
            {competitions.map((comp, idx) => (
              ...
              >
                ...
                <div className="absolute inset-0 z-0 bg-gradient-to-t from-gray-900/70 via-gray-900/30 to-transparent" />
                <div className="relative z-10 p-6 flex flex-col h-full justify-end">
                  <div className="flex items-center mb-2">
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mr-2 ${comp.is_completed ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
        ...
        {modalOpen && selectedCompetition && (
          ...
          >
            ...
            >
              ...
              <div className="mb-2">
                <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${selectedCompetition.is_completed ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
