...
function NotificationManagement() {
  ...
  const fetchNotifications = async () => {
    ...
    setNotifError('');
    try {
      const res = await fetch('https://kochchibazaar.lk/api/notifications/', {
  ...
  const handleDeleteNotification = async (id) => {
    ...
    const token = localStorage.getItem('admin_token');
    try {
      const res = await fetch(`https://kochchibazaar.lk/api/notifications/${id}`, {
        method: 'DELETE',
        headers: {
          accept: '*/*',
          Authorization: `Bearer ${token}`,
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border p-6 mb-8 max-w-lg mx-auto space-y-4 shadow">
        <div>
        ...
        <button
          type="submit"
          className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-xl font-semibold transition-colors w-full"
          disabled={loading}
        >
