...
function CompetitionManagement() {
  ...
  const fetchCompetitions = async () => {
    ...
    setError('');
    try {
      const res = await fetch('https://kochchibazaar.lk/api/competition/', {
  ...
  const handleAddWinner = async (competitionId, e) => {
    ...
    try {
      const res = await fetch(`https://kochchibazaar.lk/api/competition/${competitionId}/winners`, {
        method: 'POST',
        headers: {
          accept: 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteCompetition = async (competitionId) => {
    ...
    const token = localStorage.getItem('admin_token');
    try {
      const res = await fetch(`https://kochchibazaar.lk/api/competition/${competitionId}`, {
        method: 'DELETE',
        headers: {
          accept: '*/*',
          Authorization: `Bearer ${token}`,
  ...
  return (
    <div>
      ...
      {showCreateForm && (
        <form onSubmit={handleCreateCompetition} className="bg-white rounded-xl border p-6 mb-8 space-y-4">
          <div>
                ...
                {!comp.is_completed && (
                  <button
                    ...
                    onClick={async () => {
                      const token = localStorage.getItem('admin_token');
                      try {
                        const res = await fetch(`https://kochchibazaar.lk/api/competition/${comp.id}/complete`, {
                          method: 'PUT',
                          headers: {
                            accept: 'application/json',
                            Authorization: `Bearer ${token}`,
