...
import { FaUserCog, FaUsers, FaHourglassHalf, FaCheckCircle, FaBlog, FaTrophy, FaBell, FaAd, FaPlus, FaTrash, FaDownload, FaBold, FaListUl, FaListOl, FaUnderline, FaItalic, FaAlignLeft, FaAlignCenter, FaAlignRight, FaEdit, FaImage, FaSignOutAlt } from 'react-icons/fa';
...
function AdminPage() {
  ...
  const [approvedAds, setApprovedAds] = useState([
    { id: 3, shop: 'Shop C', phone: '777888999', email: 'shopc@example.com' },
  ]);
  const [popupAds, setPopupAds] = useState([]);

  const fetchPopupAds = async () => {
    try {
      const res = await fetch('https://kochchibazaar.lk/api/popup-ads/');
  ...
  const fetchAdminList = async () => {
    ...
    try {
      const response = await fetch('https://kochchibazaar.lk/api/auth/users/last-logins', {
  ...
  const handleAddSubAdmin = async () => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch('https://kochchibazaar.lk/api/auth/create-admin', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        fetchAdminList(); // Refresh admin list after creation
      } else {
        const errorData = await response.json();
        alert(`Error: ${errorData.detail || 'Failed to create sub-admin'}`);
      }
    } catch (error) {
  ...
  const handleDeletePopupAd = async (id) => {
    try {
      const res = await fetch(`https://kochchibazaar.lk/api/popup-ads/${id}`, {
        method: 'DELETE',
      });
