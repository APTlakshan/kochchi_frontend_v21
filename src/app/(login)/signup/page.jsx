...
function SignUpPage() {
  ...
  const handleSignUp = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('https://kochchibazaar.lk/api/auth/register', {
        method: 'POST',
  ...
  const handleVerify = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('https://kochchibazaar.lk/api/auth/verify-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', accept: 'application/json' },
  ...
  const handleResend = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('https://kochchibazaar.lk/api/auth/resend-verification', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', accept: 'application/json' },
          ...
          {step === 'signup' && (
            <form onSubmit={handleSignUp}>
              ...
              <motion.div className="mb-6" variants={itemVariants}>
                ...
                <input type="password" id="confirmPassword" value={form.confirmPassword} onChange={handleChange} placeholder="********" className="w-full px-4 py-3 border rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-200 text-gray-800" required />
              </motion.div>
